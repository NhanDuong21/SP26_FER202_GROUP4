...
function Sidebar({ isCollapsed, isMobileOpen, onCloseMobile }) {
  ...
  return (
    ...
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-50 h-screen shrink-0 overflow-hidden bg-[#0e1b46] text-white transition-all duration-300 lg:sticky lg:z-30 ${isCollapsed ? "w-[96px]" : "w-[290px]"
          } ${isMobileOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0`}
      >
        <div className="flex h-full flex-col">
          <div
            className={`border-b border-white/10 ${isCollapsed ? "px-4 py-5" : "px-7 py-5"
              }`}
          >
          ...
          <div
            className={`border-b border-white/10 ${isCollapsed ? "px-3 py-6" : "px-7 py-6"
              }`}
          >
            <div
              className={`flex items-center ${isCollapsed ? "justify-center" : "gap-4"
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={onCloseMobile}
                    className={({ isActive }) =>
                      `group flex items-center rounded-2xl transition-all duration-200 ${isCollapsed
